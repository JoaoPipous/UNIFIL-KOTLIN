...
class ListActivity : AppCompatActivity() {
    ...
    // Corrigido: token agora tem valor padrão (evita lateinit exception)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Se token não existir, encerra a tela e avisa
        if (token.isEmpty()) {
            ...
            override fun onResponse(call: Call<List<TaskResponse>>, response: Response<List<TaskResponse>>) {
                if (response.isSuccessful) {
                    val tasks = response.body()
                        ?.filter { it.data?.description != null }
                        ?.map { it.data!!.description } ?: emptyList()
